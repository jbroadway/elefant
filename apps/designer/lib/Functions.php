...
function invalid_php_functions ($text) {
	...
	if (preg_match ('/\b(' . join ('|', $invalid_functions) . ')\s*?\(/s', $text, $regs)) {
		return true;
	}
	
	// Also check for base64-encoded versions
	foreach ($invalid_functions as $fn) {
		if (strpos ($text, base64_encode ($fn)) !== false) {
			return true;
		}
	}
