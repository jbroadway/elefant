...
class VersionsTest extends PHPUnit_Framework_TestCase {
	...
	static function setUpBeforeClass () {
		...
		if (! DB::execute ('create table elefant_versions (
		...
		if (! DB::execute ('create index elefant_versions_class on elefant_versions (class, pkey, ts)')) {
			die ('Failed to create versions_class index');
		}
		if (! DB::execute ('create index elefant_versions_user on elefant_versions (user, ts)')) {
	...
	function test_add () {
		...
		$this->assertEquals (DB::shift ('select count(*) from elefant_versions'), 1);
	...
	function test_diff () {
		...
		$this->assertEquals (DB::shift ('select count(*) from elefant_versions'), 2);
	...
	function test_recent_user () {
		// recent with user
		DB::execute ('update elefant_versions set user = 1');
